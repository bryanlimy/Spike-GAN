...
flags = tf.app.flags
flags.DEFINE_string("architecture", "conv",
                    "semi-conv (conv) or fully connected (fc)")
...
flags.DEFINE_string(
    "recovery_dir", "",
    "in case the real samples are already stored in another folder")
flags.DEFINE_boolean("is_train", False,
                     "True for training, False for testing [False]")
flags.DEFINE_integer(
    "training_step", 200,
    "number of batches between weigths and performance saving")
flags.DEFINE_string("training_stage", '',
                    "stage of the training used for the GAN")
...
flags.DEFINE_integer("critic_iters", 5,
                     "number of times the discriminator will be updated")
flags.DEFINE_float("lambd", 10, "parameter gradient penalization")

#parameter set specifiying data
flags.DEFINE_string(
    "sample_dir", "",
    "where the samples will be saved. This is automatically defined below, here I just initialize the field."
)
flags.DEFINE_string("dataset", "uniform",
                    "type of neural activity. It can be simulated  or retina")
flags.DEFINE_string("data_instance", "1",
                    "if data==retina, this allows chosing the data instance")
...
flags.DEFINE_string(
    "iteration", "0",
    "in case several instances are run with the same parameters")
flags.DEFINE_integer(
    "ref_period", 2,
    "minimum number of ms between spikes (if < 0, no refractory period is imposed)"
)
flags.DEFINE_float("firing_rate", 0.25,
                   "maximum firing rate of the simulated responses")
flags.DEFINE_float("correlation", 0.9, "correlation between neurons")
flags.DEFINE_integer(
    "group_size", 4,
    "size of the correlated groups (e.g. 2 means pairwise correlations)")
flags.DEFINE_float("noise_in_packet", 0,
                   "std of gaussian noise added to packets")
flags.DEFINE_integer(
    "number_of_modes", 1,
    "[1,2] Number of different responses in the packet simulation. If =2 each type \
